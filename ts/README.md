# Typeform TypeScript SDK



The TypeScript SDK for the Typeform API — a type-safe, entity-oriented client with full async/await support.
...
- Releases: [https://github.com/voxgig-sdk/typeform-sdk/releases](https://github.com/voxgig-sdk/typeform-sdk/releases)
...
`list()` resolves to an array of Form ENTITIES — every operation
...
```ts
try {
  const form = await client.Form().load({ id: 'example_id' })
  console.log(form)
} catch (err) {
...
```ts
// Create — returns the created Form ENTITY (.data() for the record)
