...
const TEST_SEED = { "entity": { "form": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+TypeformSDK\b/.test(code) ||
        /\bTypeformSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+TypeformSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/TypeformSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
