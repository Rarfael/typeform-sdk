...
[MCP](https://modelcontextprotocol.io) server exposing the Typeform SDK as
two agent tools — `typeform_list` and `typeform_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/typeform-mcp)
make build

# 2. Provide credentials via the environment
export TYPEFORM_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user typeform \
  -- /absolute/path/to/typeform-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./typeform-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// typeform_list: first page of records
{ "entity": "form" }
{ "entity": "form", "query": { } }

// typeform_load: one record by id
{ "entity": "form", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/typeform-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user typeform \
     -- "$PWD"/dist/*/typeform-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `typeform_list` and `typeform_load` tools now appear
   in new sessions. Ask the agent to *"list form using typeform"*
   and it calls `typeform_list` with `{"entity":"form"}`.
...
```sh
export TYPEFORM_APIKEY=sk_live_xxx            # API key
export TYPEFORM_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `typeform_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `typeform_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `TYPEFORM_APIKEY` | API key sent with every request. |
| `TYPEFORM_BASE` | Optional override of the API base URL. |
...
```sh
./typeform-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"typeform_load","arguments":{"entity":"form","query":{"id":1}}}}'
```
