// Command Typeform-cli is a CLI + REPL for the Typeform SDK,
...
const prompt = "typeform"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "form"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      TYPEFORM_APIKEY (api key), TYPEFORM_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: TYPEFORM_APIKEY carries the API key and
	// TYPEFORM_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("TYPEFORM_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("TYPEFORM_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewTypeformSDK(opts)
