...
boru-driven command-line client **and** interactive REPL for the Typeform
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/typeform-cli)
make build
...
./typeform-cli list form
./typeform-cli load 1 form            # {id:1} shorthand
./typeform-cli load '{id:1}' form       # explicit match map
./typeform-cli update '{name:"x"}' form

# 5. Override the API base URL for a single call
TYPEFORM_BASE=https://api.example.com ./typeform-cli list form
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/typeform-cli
   ```
...
```sh
./typeform-cli load 1 form          # scalar shorthand for {id:1}
./typeform-cli load '{id:1}' form     # explicit match map
```
...
```sh
./typeform-cli update '{id:1,name:"new"}' form
```
...
Run with no arguments to open a REPL (prompt `typeform>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `form`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/typeform-cli`. |
...
(`./typeform-cli <expr>`) and interactively (the REPL), and expressions compose the
