...
class FormDirectTest extends TestCase
{
    public function test_direct_list_form(): void
    {
        $setup = form_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-form", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
    ...
    public function test_direct_load_form(): void
    {
        $setup = form_direct_setup(["id" => "direct01"]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-load-form", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
...
function form_direct_setup($mockres)
{
    ...
    if ($live) {
        ...
        $client = new TypeformSDK($merged_opts);
        return [
            "client" => $client,
            "calls" => $calls,
            "live" => true,
