...
{
    private static function utility(): TypeformUtility
    {
        return new TypeformUtility();
    }
    ...
    private static function ctx(array $over = []): TypeformContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new TypeformContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new TypeformOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof TypeformError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new TypeformOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = TypeformMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new TypeformOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = TypeformMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new TypeformOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = TypeformMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TypeformSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = TypeformMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new TypeformResponse([]),
            'result' => new TypeformResult([]),
        ]);
        [, $err] = TypeformMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new TypeformSpec([]),
            'response' => null,
            'result' => new TypeformResult([]),
        ]);
        [, $err] = TypeformMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new TypeformSpec(['step' => 's']),
            'response' => new TypeformResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new TypeformResult([]),
        ]);
        [, $err] = TypeformMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new TypeformSpec(['step' => 's']),
            'response' => new TypeformResponse(self::resp(200, ['v' => 1])),
            'result' => new TypeformResult([]),
        ]);
        [, $err] = TypeformMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new TypeformSpec(['step' => 's']),
            'response' => new TypeformResponse(self::resp(200, ['v' => 2])),
            'result' => new TypeformResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        TypeformMakeResponse::call($ctx);
    ...
    {
        $preset = new TypeformResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new TypeformSpec([]),
            'response' => new TypeformResponse([]),
            'result' => new TypeformResult([]),
        ...
        [$got, $err] = TypeformMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new TypeformResult([])]);
        [, $err] = TypeformMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new TypeformSpec([]), 'result' => null]);
        [, $err] = TypeformMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TypeformSpec(['step' => 's']),
            'result' => new TypeformResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new TypeformOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TypeformMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TypeformSpec(['step' => 's']),
            'result' => new TypeformResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new TypeformOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TypeformMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TypeformResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new TypeformSpec([]),
            'result' => new TypeformResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = TypeformMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TypeformMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new TypeformError('boom', 'boom');
        $utility->fetcher = function (TypeformContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TypeformSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = TypeformMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (TypeformContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = TypeformMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(TypeformResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (TypeformContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TypeformSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = TypeformMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new TypeformResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new TypeformSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = TypeformMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = TypeformMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = TypeformMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new TypeformResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, TypeformDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new TypeformResult(['ok' => false])]);
        $this->expectException(TypeformError::class);
        TypeformDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new TypeformResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', TypeformMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        TypeformMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new TypeformBaseFeature();
        $b = new TypeformBaseFeature();
        TypeformFeatureAdd::call($ctx, $a);
        TypeformFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): TypeformBaseFeature
    {
        $f = new TypeformBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        TypeformFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): TypeformContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new TypeformSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TypeformPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new TypeformResponse(['headers' => null]),
            'result' => new TypeformResult([]),
        ]);
        TypeformResultHeaders::call($ctx);
