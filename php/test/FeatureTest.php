...
class FtCtrl extends TypeformControl
{
...
class FtClient
{
    ...
    private TypeformUtility $utility;

    public function __construct(TypeformUtility $utility, array $options)
    {
    ...
    public function get_utility(): TypeformUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (TypeformContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public TypeformContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (TypeformContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = TypeformConfig::make_feature($name);
    ...
    private static function buildUrl(TypeformSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof TypeformError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new TypeformResponse($fetched);
        }
    ...
    private function fail(TypeformContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (TypeformContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
