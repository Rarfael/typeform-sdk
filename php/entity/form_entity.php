...
class FormEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new FormEntity($this->_client, $opts);
    }

    /**
     * @param Form|array $args Form data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = TypeformHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Form|array The current Form data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Form fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = TypeformHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Form fields).
     */
     ...
     * Load a single Form.
     *
     * @param FormLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed FormLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Form|array The loaded Form as an assoc-array at the
     *   SDK boundary; throws TypeformError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = TypeformHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * List Form items matching the given filter.
     *
     * @param FormListMatch|array|null $reqmatch Match filter (any subset
     *   of Form fields) as an assoc-array; FormListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Form[]|array A list of Form items as assoc-arrays at
     *   the SDK boundary; throws TypeformError on failure (item-5 convention).
     */
     ...
     * Create a new Form.
     *
     * @param FormCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed FormCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Form|array The created Form as an assoc-array at the
     *   SDK boundary; throws TypeformError on failure (item-5 convention).
     */
     ...
     *
     * @param FormUpdateData|array|null $reqdata Body data as an assoc-array;
     *   a typed FormUpdateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Form|array The updated Form as an assoc-array at the
     *   SDK boundary; throws TypeformError on failure (item-5 convention).
     */
    ...
    /**
     * Remove an Form matching the given criteria.
     *
     * @param FormRemoveMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; FormRemoveMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Form|array The removed Form as an assoc-array at the
     *   SDK boundary; throws TypeformError on failure (item-5 convention).
     */
