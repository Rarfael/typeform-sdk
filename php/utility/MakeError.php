...
class TypeformMakeError
{
    public static function call(?TypeformContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new TypeformContext([], null);
        }
        $op = $ctx->op ?? new TypeformOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new TypeformResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof TypeformError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new TypeformError('', $msg, $ctx);
        ...
        if ($err instanceof TypeformError) {
