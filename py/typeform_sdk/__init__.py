...
from typeform_sdk.utility import register

# Load features
from typeform_sdk.feature.base_feature import TypeformBaseFeature
from typeform_sdk.features import _has_feature, _make_feature
...
class TypeformSDK:
    ...
    def get_utility(self):
        return TypeformUtility.copy(self._utility)
    ...
    def graphql(self, query, variables=None, ctrl=None):
        ...
        if isinstance(errors, list) and 0 < len(errors):
            ...
            res["ok"] = False
            res["err"] = Exception("TypeformSDK: graphql: " + str(msg))
            res["graphql"] = errors

        return res


    def Form(self, data=None) -> "FormEntity":
        """Entity factory: client.Form().list() / client.Form().load({"id": ...})."""
        from typeform_sdk.entity.form_entity import FormEntity
        return FormEntity(self, data)



    @classmethod
    def test(cls, testopts=None, sdkopts=None) -> "TypeformSDK":
        if sdkopts is None:
            sdkopts = {}
