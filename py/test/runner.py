...
class TypeformTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            TypeformTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = TypeformTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = TypeformTestRunner.getenv("TYPEFORM_TEST_LIVE")
        override = TypeformTestRunner.getenv("TYPEFORM_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = TypeformTestRunner.getenv(key)
        ...
        explain = TypeformTestRunner.getenv("TYPEFORM_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["TYPEFORM_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if TypeformTestRunner._test_control is not None:
            return TypeformTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                TypeformTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            TypeformTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = TypeformTestRunner.load_test_control()
...
def load_env_local():
    TypeformTestRunner.load_env_local()


def env_override(m):
    return TypeformTestRunner.env_override(m)


def entity_data(v):
    return TypeformTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return TypeformTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return TypeformTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return TypeformTestRunner.load_test_control()


def live_delay_ms():
    return TypeformTestRunner.live_delay_ms()
